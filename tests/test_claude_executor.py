...
class TestIsClaudeAvailable:
    def test_returns_true_when_claude_in_path(self):
        with patch("harness.services.claude_executor.shutil.which", return_value="/usr/local/bin/claude"):
            assert is_claude_available() is True

    def test_returns_false_when_not_in_path(self):
        with patch("harness.services.claude_executor.shutil.which", return_value=None):
...
class TestRunClaudeImplement:
    def test_returns_true_on_zero_exit(self, tmp_path):
        ...
        mock_result.stderr = ""
        with patch("harness.services.claude_executor.subprocess.run", return_value=mock_result) as mock_run:
    ...
    def test_returns_false_on_nonzero_exit(self, tmp_path):
        ...
        with patch("harness.services.claude_executor.subprocess.run", return_value=mock_result):
    ...
    def test_returns_false_on_timeout(self, tmp_path):
        with patch(
            "harness.services.claude_executor.subprocess.run",
...
class TestCaptureDiffStaged:
    def test_returns_diff_for_existing_files(self, tmp_path):
        ...
        with patch(
            "harness.services.claude_executor.subprocess.run",
            side_effect=[add_result, diff_result, reset_result],
        ) as mock_run:
    ...
    def test_returns_empty_when_no_files_exist(self, tmp_path):
        with patch("harness.services.claude_executor.subprocess.run") as mock_run:
...
class TestResetAllowedFiles:
    def test_restores_tracked_file(self, tmp_path):
        ...
        with patch(
            "harness.services.claude_executor.subprocess.run",
            side_effect=[ls_result, checkout_result],
        ) as mock_run:
    ...
    def test_deletes_new_untracked_file(self, tmp_path):
        ...
        with patch("harness.services.claude_executor.subprocess.run", return_value=ls_result):
